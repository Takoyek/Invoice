...
def process_text(line):
    ...
    elif "بیست گیگ" in line and not any(phrase in line for phrase in ["صد و بیست گیگ", "صدو بیست گیگ", "صدوبیست گیگ"]):
        return re.sub(r"✅", "✅  [35]", line)

    elif "سی گیگ" in line:
        return re.sub(r"✅", "✅  [45]", line)

    elif "چهل گیگ" in line and not any(phrase in line for phrase in ["صد و چهل گیگ", "صدو چهل گیگ", "صدوچهل گیگ"]):
        if any(day in line for day in ["شصت روز", "شصد روز"]):
            return re.sub(r"✅", "✅  [70]", line)
        else:
            return re.sub(r"✅", "✅  [55]", line)

    elif "پنجاه گیگ" in line and not any(phrase in line for phrase in ["صد و پنجاه گیگ", "صدو پنجاه گیگ", "صدوپنجاه گیگ"]):
        return re.sub(r"✅", "✅  [65]", line)

    elif any(gig in line for gig in ["شصت گیگ", "شصد گیگ"]) and \
    ...
    elif "هفتاد گیگ" in line:
        return re.sub(r"✅", "✅  [91]", line)

    elif "هشتاد گیگ" in line and not any(phrase in line for phrase in ["صد و هشتاد گیگ", "صدو هشتاد گیگ", "صدوهشتاد گیگ"]):
        if any(day in line for day in ["شصت روز", "شصد روز"]):
            return re.sub(r"✅", "✅  [110]", line)
        else:
            return re.sub(r"✅", "✅  [104]", line)

    elif "نود گیگ" in line:
        if any(day in line for day in ["شصت روز", "شصد روز"]):
    ...
    elif any(phrase in line for phrase in ["صد و بیست گیگ", "صدو بیست گیگ", "صدوبیست گیگ"]):
    ...
    elif any(phrase in line for phrase in ["صد و پنجاه گیگ", "صدو پنجاه گیگ", "صدوپنجاه گیگ"]):
