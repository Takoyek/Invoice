...
def process_text(line):
    ...
    mappings = [
        (rf"\bسیصد {GIG}\b.*?(180 روز|{SAD}هشتاد روز)", "[510]"),  # 300G 180R        
        (rf"\bدویست {GIG}\b.*?(120 روز|{SAD}{BIST} روز)", "[340]"),  # 200G 120R
        (rf"{SAD}هشتاد {GIG}.*?(180 روز|{SAD}هشتاد روز)", "[360]"),  # 180G 180R 
        (rf"{SAD}پنجاه {GIG}.*?{NVD_R}", "[250]"),  # 150G 90R      
        (rf"{SAD}پنجاه {GIG}", "[240]"),  # 150G
        (rf"{SAD}{BIST} {GIG}.*?(120 روز|{SAD}{BIST} روز)", "[240]"),  # 120G 120R
        (rf"{SAD}{BIST} {GIG}.*?{NVD_R}", "[225]"),  # 120G 90R
        (rf"{SAD}{BIST} {GIG}", "[190]"),  # 120G 30R 60R
        (rf"\bصد {GIG}\b.*?{NVD_R}", "[200]"),  # 100G 90R
        (rf"\bصد {GIG}\b.*?({SHST_R})", "[171]"),  # 100G 60R       
        (rf"\bصد {GIG}\b", "[160]"),  # 100G 30R 60R
        (rf"نود {GIG}.*?{NVD_R}", "[180]"),  # 90G 90R
        (rf"نود {GIG}.*?({SHST_R})", "[168]"),  # 90G 60R
        (rf"نود {GIG}", "[145]"),  # 90G
